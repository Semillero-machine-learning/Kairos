...
| RNF-02 | **Arranque en frío.** El backend en el plan gratuito de Render se suspende tras ~15 minutos de inactividad y la primera petición puede tardar cerca de un minuto. El frontend debe mostrar un estado de carga explícito ("Despertando el servidor...") tras 3 segundos de espera, con reintento automático y tiempo de espera de 90 segundos. No hay ping de mantenimiento: se retiró porque fallaba por tiempo agotado y generaba avisos constantes. Que el servicio duerma es aceptable; el aviso explícito es la medida. |
...
| RNF-07 | **Suspensión de Supabase.** El plan gratuito pausa proyectos tras una semana sin actividad. El proceso diario de recordatorios despierta la API antes de disparar el trabajo, y ese `/health` ejecuta un `SELECT 1`: mientras ese cron corra, no puede haber una semana sin actividad. |
